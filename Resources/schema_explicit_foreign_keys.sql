...
Resources/schema_efficient.sql has another version of this code, that merges the foreign keys into the first selection lines. That is:
    emp_title_id varchar NOT NULL,
    FOREIGN KEY (emp_title_id) REFERENCES title (title_id)
                becomes:
    emp_title_id varchar NOT NULL REFERENCES title (title_id)
I like it better, and it certainly works perfectly, but QuickDBD (https://app.quickdatabasediagrams.com/#/) does not import it and show the relationships

The root repo holds schema.sql, which creates the tables but does not import the data (use the pgAdmin GUI to import each .csv file to each table with that file)
*/
